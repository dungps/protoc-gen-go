...
import (
	"fmt"
	"github.com/dungps/protoc-gen-go/options"
	"go/ast"
	"go/parser"
	"go/token"
	...
	"github.com/dungps/protoc-gen-go/internal/encoding/tag"
	"github.com/dungps/protoc-gen-go/internal/genid"
...
func genMessageField(g *protogen.GeneratedFile, f *fileInfo, m *messageInfo, field *protogen.Field, sf *structFields) {
	...
	isExtendStruct := proto.GetExtension(opt, options.E_IsExtendStruct)

	tags := structTags{
		{"protobuf", fieldProtobufTagValue(field)},
	}

	if !toBool(isExtendStruct) {
...
func genMessageGetterMethods(g *protogen.GeneratedFile, f *fileInfo, m *messageInfo) {
	for _, field := range m.Fields {
		...
		returnName := field.GoName
		if toBool(isExtendStruct) {
			returnName = strings.Replace(goType, "*", "", 1)
		}
		...
		case field.Desc.IsWeak():
			...
			if m.isTracked {
				g.P("_ = x.", genid.WeakFieldPrefix_goname+returnName)
			}
			g.P("}")
...
func fieldGoType(g *protogen.GeneratedFile, f *fileInfo, field *protogen.Field) (goType string, pointer bool) {
	...
	case field.Desc.IsMap():
		...
		opt := field.Desc.Options()
		isValueRepeat := proto.GetExtension(opt, options.E_MapValueRepeat)
		if toBool(isValueRepeat) {
